...
rule convert_kraken2_classification:
    ...
    shell:
        ...
        # 更新kraken2的out文件，因为kraken2的out文件中，taxid是按照kraken2的taxonomy来的，而kraken2的taxonomy和当前的taxonomy不一致，所以需要更新
...
rule bracken_output:
    ...
    run:
        ...
        reads_distributed_practice = pd.read_table(input.TSV_bracken,
            usecols=['added_reads']).squeeze('columns').sum() ## added_reads列数字的总和
