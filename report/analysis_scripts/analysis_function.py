...
class ClassificationResults:
    ...
    def _normalize_series(series):
        return series / series.sum() ## 计算相对丰度，即每个分类的reads数除以总reads数

    def excl_no_hit(self, normalized=False): # 排除no_hit的结果
        results_excl_no_hit = self.results[self.results.index != 'no_hit']
        if normalized:
            return self._normalize_series(results_excl_no_hit)
        return results_excl_no_hit ## 计算除no_hit外的reads比例（相对丰度）

    def only_classified(self, normalized=False): # 只计算已经分类的物种相对丰度
        """
        Return the reads not in the list_uc.
        """
        only_classified = self.results[~self.results.index.isin(list_uc)] # 排除list_uc中的reads；~为反向操作符
        if normalized:
            return self._normalize_series(only_classified)
        return only_classified

    def bundled_misclassified_others(self, ground_truth, normalized=False): # 将结果分为三类：正确分类、错误分类（Misclassified）和其他（Others）。
        """
        'Others' --> list_uc
        ...
        groups = np.where(np.isin(self.results.index, list_uc), 'Others',
                          np.where(np.isin(self.results.index, ground_truth.index), self.results.index,
                                   'Misclassified'))
        ## 将分类结果分为三类；Others：list_uc中的taxid；Misclassified：不是list_uc中的且不在ground_truth中的taxid；其余为正确分类的taxid；保留原来的taxid(index)
    ...
    def bundled_misclassified_excl_others(self, ground_truth, normalized=False): # 排除 Others 类别，并将错误分类的单元打包。
        """
...
def plot_classification_bar(multiple_results: List[pd.Series], ground_truth, ground_truth_tax, ax=None):
    ...
    if 'Others' in multiple_results[0].index:
        colormap = mpl.colors.ListedColormap(COLOR_LIST[:len(multiple_results[0]) - 2] + ['k', 'gray']) # others设置为灰色，Misclassified设置为黑色
...
def tp_fp_fn(results: pd.Series, ground_truth: pd.Series, threshold=None) -> pd.Series:
    ...
    if threshold is None:
        ...
        class_reads_above_thresh = results[results >= threshold].index.values ## 大于阈值的物种名转化为NumPy数组
        classifier_status = np.isin(class_reads_above_thresh, ground_truth.index) # 返回布尔数组；Ture为1，False为0
...
def plot_top15(results: pd.Series, ground_truth: pd.Series, classifier_name: str, ax=None):
    ...
    ax.set_xlabel('Count')
    # ax.set_ylabel('')
    ax.ticklabel_format(axis='x', style='sci', scilimits=(0, 0), useMathText=True) # 格式化x轴；'plain'（普通格式）和 'sci'（科学计数法）
